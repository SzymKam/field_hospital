...
class TestEventResponse(TestCase):
    ...
    def test_list_patch_not_logged_user_return_403(self):
        response = self.client.patch(path=self.url_list, data=self.data)

        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.request["REQUEST_METHOD"], "PATCH")

    def test_list_patch_logged_user_return_403(self):
        self.client.force_login(user=self.user)
        response = self.client.patch(path=self.url_list, data=self.data)
    ...
    def test_detail_post_not_logged_user_return_403(self):
        response = self.client.post(path=self.url_detail, data=self.data)
