...
class AddAuthPersonTest(TestCase):
    def setUp(self) -> None:
        self.client = Client()
        ...
        self.patient_1 = PatientFactory(event=self.event)
        self.patient_2 = PatientFactory(event=self.event)

        self.url = reverse("add-auth-person", kwargs={"event": self.event.id, "pk": self.patient_2.id})
