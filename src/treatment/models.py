...
from .constants import MEDICAL_QUALIFICATIONS, DRUG_AND_FLUID_CHOICES, DRUG_DOSAGE_FORM, FLUID_VOLUME, VITAL_SIGN_NAME
...
class MedicalStaff(models.Model):
    ...
    medical_qualifications = models.CharField(choices=MEDICAL_QUALIFICATIONS,
                                              default="First aid",
                                              help_text="Qualifications of medic", max_length=20)


class Drug(models.Model):
    name = models.CharField(choices=DRUG_AND_FLUID_CHOICES, blank=True, help_text="Add new drug", max_length=40)
    dosage_form = models.CharField(choices=DRUG_DOSAGE_FORM, blank=True, help_text="Dosage form of drug", max_length=20)
...
class Treatment(models.Model):
    ...
    vital_sign = models.ForeignKey(VitalSign,
                                   blank=True,
                                   help_text="Vital signs of patient",
                                   max_length=50,
                                   on_delete=models.CASCADE)
    drugs = models.ForeignKey(Drug,
                              blank=True,
    ...
    diagnosis = models.CharField(blank=True, help_text="Diagnose", max_length=50)  # todo maybe add icd10 library??
    additional_info = models.TextField(blank=True, help_text="Additional info about patient treatment")
    medical_staff = models.ForeignKey(MedicalStaff,
                                      on_delete=models.SET("No staff assigned"),
